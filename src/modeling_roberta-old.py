...
class RobertaEncoder(nn.Module):
    def __init__(self, config, fusion_layer=None, dim_common=256, n_attn_heads=1):
        super().__init__()
        self.config = config
        ...
        self._linear_4 = nn.Linear(fusion_dim + dim_common, fusion_dim)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i == self.fusion_layer:
                ...
                attn_output, _ = self._multi_head_attn_1(Q, K, V) #0或NaN
                #print("attn_output="+str(attn_output))
                attn_output = attn_output.transpose(0, 1)
                output = self._linear_4(torch.cat((hidden_states, attn_output), 2))
                #print("output="+str(output))
                # raise Exception  # 注释掉调试异常
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True, fusion_layer=None, dim_common=256, n_attn_heads=1):
        super().__init__(config)
        self.config = config

        self.embeddings = RobertaEmbeddings(config)
        self.encoder = RobertaEncoder(config, fusion_layer, dim_common, n_attn_heads)
