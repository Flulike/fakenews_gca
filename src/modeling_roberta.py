...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if position_ids is None:
            if input_ids is not None:
                # Standard BERT position IDs: simple sequence from 0 to seq_length
    ...
    def create_position_ids_from_inputs_embeds(self, inputs_embeds):
        ...
        position_ids = torch.arange(
            0, sequence_length, dtype=torch.long, device=inputs_embeds.device
        )
...
class RobertaEncoder(nn.Module):
    def __init__(self, config, fusion_layer=None, dim_common=256, n_attn_heads=1, disable_gate=False):
        super().__init__()
        self.config = config
        ...
        self.disable_gate = disable_gate  # 添加门限控制标志
        text_feature_dim = 768 #可能要改
        fusion_dim = 768
        #self.roberta_encoder_res=RobertaEncoderResults.from_pretrained('FacebookAI/roberta-base', return_dict=True)
        self._linear_1 = nn.Linear(text_feature_dim, dim_common) # K
        self._linear_2 = nn.Linear(text_feature_dim, dim_common) # V
        self._linear_3 = nn.Linear(fusion_dim, dim_common) # Q
        self._multi_head_attn_1 = nn.MultiheadAttention(dim_common, n_attn_heads)
        self._linear_4 = nn.Linear(dim_common, fusion_dim)
        self.gate_layer = nn.Sequential(nn.Linear(fusion_dim + dim_common, text_feature_dim),
                                        nn.Tanh(),
    ...
    def forward(
        ...
        #explanation_features=explanation_enc_out.last_hidden_state #此模块有问题
        #print("explanation_features="+str(exp_feature))
        #print("explanation_features="+str(exp_feature.shape))
        #raise Exception
        # 增加解释特征 结束
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i == self.fusion_layer:
                ...
                if self.disable_gate:
                    # 禁用门限机制：直接使用注意力输出，不进行门限融合
                    gate = torch.ones_like(hidden_states[:, :, :1])  # 创建全1的门限（shape: [batch, seq, 1]）
                    hidden_states = attn_output_2 + hidden_states  # 直接使用注意力输出
                else:
                    # 正常的门限机制
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True, fusion_layer=None, dim_common=256, n_attn_heads=1, disable_gate=False):
        super().__init__(config)
        self.config = config

        self.embeddings = RobertaEmbeddings(config)
        self.encoder = RobertaEncoder(config, fusion_layer, dim_common, n_attn_heads, disable_gate)
...
class BertModel(RobertaModel):
    """BERT backbone that reuses the customized fusion-aware encoder."""

    config_class = BertConfig
    base_model_prefix = "bert"
    _no_split_modules = ["BertEmbeddings", "RobertaLayer"]

    def __init__(self, config, add_pooling_layer=True, fusion_layer=None, dim_common=256, n_attn_heads=1, disable_gate=False):
        # Call grandparent init to avoid RobertaModel's embedding initialization
        RobertaPreTrainedModel.__init__(self, config)
        self.config = config

        # Use BertEmbeddings instead of RobertaEmbeddings
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config, fusion_layer, dim_common, n_attn_heads):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config

        self.roberta = RobertaModel(config, add_pooling_layer=False, fusion_layer=None, dim_common=256, n_attn_heads=1)
